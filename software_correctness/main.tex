...
\frame[plain]{\titlepage
    \note[itemize]{
        \item The goal of the presentation is to expose you to topics that appear casually in academia, but are not often discussed in the industry.
        \item This talk will be very different from all other ones, there will be no Dart nor Flutter.
        \item Not sure why this talk was accepted for the conference.}
}
...
\begin{frame}[fragile]
    ...
    \note[item]{The word "proof" will appear more often now. A proof is a formal guarantee that a certain property holds.}
\end{frame}
...
\begin{frame}{Programs are proofs}
    ...
    Proofs are nothing more than a program that is type checked. \note[item]{For example, in Dart if you have a variable with value 5 and type int, then you have a proof that 5 is an int.}
\end{frame}
